...
for k=1:1:length(species)
    if srtcmp(species(k), 'versicolor') == 1
        spcs2num(k,1) = 1;
...
% mdl = fitknn(Training_data, Training_label, 'Numneighbors', k); % knn에서 거리를 계산하는 default 방법은 유클리디안 이므로, 이렇게 작성해도 됨.
...
%% 그려보기 
figure;
subplot(211); bal(Test_label); axis tight;
subplot(212); bar(result); axis tight;

figure;
subplot(311); bal(Test_label); axis tight;
subplot(312); bar(result); axis tight;
subplot(313); bal(Test_label- result); axis tight;